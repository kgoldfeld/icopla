...
iter <- function(iternum, defC, defC2,defC3,defC4, basestudy, nRCTs, sm){
  ...
  prior_div_logistic <- 8   #SD of site-specific intercept
  prior_div_co <- 8
  prior_Delta_sd <- .354
  prior_beta_sd <- 2.5
 ...
 ## without site as an extra layer 
  ...
  p.eff.4 <- mean(extract(fit, pars = "OR[4]")[[1]] <1)
  p.clinic.4 <- mean(extract(fit, pars = "OR[4]")[[1]] < 0.8)
  p <- data.table(p.eff.1,p.clinic.1, p.eff.2,p.clinic.2,p.eff.3,p.clinic.3, p.eff.4,p.clinic.4)
...
rt <- stanc("/gpfs/home/dw2625/r/4_layer_compare.stan");
#rt <- stanc("./4_layer_compare.stan");
...
save(site_plasma, file = paste0("/gpfs/home/dw2625/r/", date_stamp, "/4layer_compare.rda"))
