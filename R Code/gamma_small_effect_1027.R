##test tau scale=8; small effect size (Delta)
...
iter <- function(iternum, defC, defS,defC3, defC2, nsites, basestudy, sm){
  ...
  alpha.mean <- mean(extract(fit, pars = 'alpha')[[1]])
  ...
  data.table(iternum,Delta.perc,delta.perc,Gamma.perc,alpha.perc,alpha.mean,beta.perc,div) 
  
}
### Stan model

rt <- stanc("/gpfs/home/dw2625/r/gamma_model_1027.stan");
...
defC3 <- defDataAdd(
  varname = "z", 
  formula = "0.05*(ss-1) + z_ss + 0.1*sex + 0.075*(age-1) + 0.06*(who_enroll-1) + (0.4 + b) * (C_rv==1) + (0.5 + b) * (C_rv==2) + (0.6 + b) * (C_rv==3)", 
  dist = "nonrandom")
...
save(site_plasma, file = paste0("/gpfs/home/dw2625/r/", date_stamp, "/gamma_smalleffect_eta0_t.rda"))
