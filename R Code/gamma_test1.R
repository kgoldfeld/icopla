...
iter <- function(iternum, defC, defC2, defS,defC3, basestudy, nsites, sm){
  ...
  sex <- dind$sex 
  x <- model.matrix(ordY ~ factor(who_enroll) + factor(age) + factor(sex), data = dind)[, -1]
  D <- ncol(x)
  ...
  colnames(beta.perc) <- c("enroll_5","enroll_6","age_2","age_3","male")
...
# beta_age = c(0, .075, .15) and beta_who = c(0, .06, .12) 
defC3 <- defDataAdd(
  varname = "z", 
  formula = " 0.1*sex + 0.075*(age-1) + 0.06*(who_enroll-1) + z_ss + (0.4 + b) * (C_rv==1) + (0.5 + b) * (C_rv==2) + (0.6 + b) * (C_rv==3)", 
  dist = "nonrandom")
...
save(site_plasma, file = paste0("/gpfs/home/dw2625/r/", date_stamp, "/no_ss_cov_gamma.rda"))
