...
defA1 <- defDataAdd(varname = "z", 
                    formula = "(0.2 + b ) * (C_rv==1) + (0.3 + b ) * (C_rv==2) + (0.4 + b ) * (C_rv==3)", 
                    dist = "nonrandom")
...
save(site_plasma, file = paste0("/gpfs/home/dw2625/R/x86_64-redhat-linux-gnu-library/file_danni/", date_stamp, "/interim_nc_234.rda"))
