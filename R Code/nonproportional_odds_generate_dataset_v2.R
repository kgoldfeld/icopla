...
npAdj <-  c(1,3,0,0.5,0.5,2,3,0.5,0.5,2,0)
setkey(dind, "id")
dl <- lapply(1:nsites, function(i) {
  b <- basestudy[i,]
  dx <- dind[study == i]
  dx <- genOrdCatNP(dx, adjVar = "z", b, npVar="control",npAdj = npAdj)
  dx[, ordY := factor(cat, levels = c(1:11))]
  dx[]
})
...
save(dind,file="./data/by_trt/dind_np_local_v3.rda")

load(file="./data/by_trt/dind_np_local_v3.rda")
...
save(fit_co,file="./data/by_trt/fit_co_np_v3.rda")

#######draw from posterior iteration#################
draws_dt <- data.table(as_draws_df(fit_co$draws()))

save(draws_dt,file="./data/by_trt/draws_dt_np_v3.rda")
