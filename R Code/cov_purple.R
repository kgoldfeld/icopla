...
iter <- function(iternum, defC, defC2, basestudy, nsites, sm){
  ...
  studydata <- list(
    ...
    age=age,WHO_enroll=WHO_enroll,sex=sex,ss=ss,
  ...
  colnames(beta_age.perc) <- c("age_1","age_2","age_3") 
  beta_sex.perc <- apply(extract(fit, pars = 'beta_sex')[[1]],2,function(x) quantile(x, probs = c(0.025, 0.25, 0.5, 0.75, 0.975)))
  colnames(beta_sex.perc) <- c("female","male") 
  beta_enroll.perc <- apply(extract(fit, pars = 'beta_enroll')[[1]],2,function(x) quantile(x, probs = c(0.025, 0.25, 0.5, 0.75, 0.975)))
  colnames(beta_enroll.perc) <- c("enroll_4","enroll_5","enroll_6") 
  beta_ss.perc <- apply(extract(fit, pars = 'beta_ss')[[1]],2,function(x) quantile(x, probs = c(0.025, 0.25, 0.5, 0.75, 0.975)))
  colnames(beta_ss.perc) <- c("ss_1","ss_2","ss_3","ss_4") 
  
  data.table(iternum,p.eff, p.clinic,Delta.perc,delta.perc,beta_ss.perc,beta_age.perc,beta_sex.perc,beta_enroll.perc,div) 
}

rt <- stanc("/gpfs/home/dw2625/r/covariates_beta_normal.stan");
...
defC2 <- defDataAdd(defC2, varname = "z", 
                    formula = "0.1*ss + 0.1*sex + 0.15*age + 0.2*WHO_enroll + (0.3 + b ) * (C_rv==1) + (0.4 + b ) * (C_rv==2) + (0.5 + b ) * (C_rv==3)", 
                    dist = "nonrandom")
...
save(site_plasma, file = paste0("/gpfs/home/dw2625/r/", date_stamp, "/cov_beta_normal_1080.rda"))
