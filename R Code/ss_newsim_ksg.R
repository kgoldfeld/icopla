...
defC3 <- defDataAdd(
  varname = "z", 
  formula = "0.1 * ss + z_ss + (0.4 + b) * (C_rv==1) + (0.5 + b) * (C_rv==2) + (0.6 + b) * (C_rv==3)", 
  dist = "nonrandom")
...
rt <- stanc("Stan Code/ss_bayesmodel_ksg.stan")
...
print(fit, pars = c("Gamma", "eta_0","delta", "Delta", "beta", "OR" ),
      probs = c(0.025, 0.25, 0.5, 0.75, 0.975),
      digits_summary = 2)

print(fit, pars = c("Gamma", "gamma","gamma_k" ),
      probs = c(0.025, 0.25, 0.5, 0.75, 0.975),
      digits_summary = 2)

print(fit, pars = c("Delta", "delta","delta_k" ),
      probs = c(0.025, 0.25, 0.5, 0.75, 0.975),
      digits_summary = 2)
