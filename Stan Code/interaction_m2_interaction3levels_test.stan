//---------------------------------------------//
  ...
  data {
    ...
    row_vector[3] ds[N];    // matrix of who_enroll N x 2 matrix
  }
...
parameters {
  ...
  vector[3] z_phi[K];      // K X 2 matrix 
  vector[3] z_delta;
  vector[D] z_beta;
  vector[3] z_gamma[3];    // 3 X 2 matrix
}
...
transformed parameters{ 
  ...
  vector[3] gamma[3];       // control-specific who_enroll effect (2 X 4 matrix)
  vector[3] gamma_k[K];     // site-specific who_enroll effect (K X 2 matrix)
...
generated quantities {
  vector[3] gd_c[3];       // control-specific who_enroll effect (3 X 2 matrix)
  vector[3] GD;
