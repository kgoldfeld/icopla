...
parameters {
  real alpha;
  real<lower=0>  eta_0[2];     // sd of delta_k (around delta_c)
  ...
  vector[D] z_beta[2];
  vector[3] z_delta[2];
  vector[K] z_rct[2];      // for RCT-specific effect 
  vector[S] z_site[2];            //for site-specific effect 
  vector[S] z_phi[2];             //site random effect
  real z_Delta[2];               // overall control effect
}
...
transformed parameters{ 
  ...
  vector[K] delta_k[2];         // RCT-specific effect
  ...
  for (i in 1:N){
     yhat[1,i]=alpha +x[i]*beta[1]+ctrl[i]*delta_site[1,site[i]];
